                  ...
                  <>
                    <span className="flex h-3.5 w-8 overflow-hidden rounded-full ring-1 ring-foreground/10">
                      {selectedTheme?.colors.map((c) => (
                        <span className="flex-1" key={c} style={{ backgroundColor: c }} />
                      ))}
                    </span>
...
function SubDrawerHeader({ title }: { title: string }) {
  return (
    <div className="flex items-center justify-between">
      <DrawerTitle className="font-semibold text-foreground text-sm">{title}</DrawerTitle>
  ...
  return (
    <button
      className={cn(
        ...
        disabled ? "cursor-not-allowed opacity-40" : "hover:bg-foreground/[0.03]"
      )}
      disabled={disabled}
