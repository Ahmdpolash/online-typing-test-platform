...
export function MobileToolbar({
  ...
  function getSubLabel() {
    ...
    if (mode === "quote") return quoteLength;
    return "zen";
  }
  ...
  return (
    <Drawer>
      ...
      >
        ...
        <Sep />
        <span className="text-primary">{getSubLabel()}</span>
        {activeModifiers.length > 0 && (
          <>
            <Sep />
            <span className="text-muted-foreground/50">{activeModifiers.join(" ")}</span>
          </>
        )}
        <Sliders className="ml-1 text-muted-foreground/30" size={14} weight="duotone" />
      </DrawerTrigger>
      ...
      <DrawerPopup className="mx-2! mb-2! flex max-h-[70dvh] flex-col rounded-2xl! [--bleed:0px]">
        <DrawerContent>
          ...
          <div className="mt-6 space-y-6">
            <DrawerSection title="Mode">
              <div className="flex flex-wrap gap-1.5">
                {MODES.map(({ value, icon: Icon, label }) => (
                  <DrawerChip active={mode === value} key={value} onClick={() => onModeChange(value)}>
                    <Icon size={14} />
                    {label}
                  </DrawerChip>
                ))}
            ...
            {mode !== "zen" && (
              <DrawerSection
                title={mode === "time" ? "Duration" : mode === "words" ? "Word count" : "Length"}
              >
                <div className="flex flex-wrap gap-1.5">
                  {mode === "time" && ([15, 30, 60, 120] as const).map((t) => (
                    <DrawerChip active={timeOption === t} key={t} onClick={() => onTimeOptionChange(t)}>{t}s</DrawerChip>
                  ))}
                  {mode === "words" && ([10, 25, 50, 100] as const).map((w) => (
                    <DrawerChip active={wordOption === w} key={w} onClick={() => onWordOptionChange(w)}>{w}</DrawerChip>
                  ))}
                  {mode === "quote" && (["short", "medium", "long"] as const).map((q) => (
                    <DrawerChip active={quoteLength === q} key={q} onClick={() => onQuoteLengthChange(q)}>{q}</DrawerChip>
                  ))}
                </div>
              </DrawerSection>
            )}
            ...
            <DrawerSection title="Difficulty">
              <div className="flex flex-wrap gap-1.5">
                <DrawerChip active={difficulty === "easy"} onClick={() => onDifficultyToggle("easy")}>
                  easy
                </DrawerChip>
                <DrawerChip active={difficulty === "hard"} onClick={() => onDifficultyToggle("hard")}>
                  hard
                </DrawerChip>
              </div>
            </DrawerSection>
