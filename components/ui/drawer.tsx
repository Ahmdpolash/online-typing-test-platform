...
function DrawerHandle({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
      className={cn("mx-auto mb-5 h-1 w-12 shrink-0 rounded-full bg-muted", className)}
      {...props}
    />
  );
}
...
function DrawerTitle({ className, ...props }: DrawerPrimitive.Title.Props) {
  return (
    <DrawerPrimitive.Title
      className={(state) =>
        cn("font-medium text-base text-foreground", resolveClassName(className, state))
      }
...
function DrawerDescription({ className, ...props }: DrawerPrimitive.Description.Props) {
  return (
    <DrawerPrimitive.Description
      className={(state) =>
        cn("mt-1.5 text-muted-foreground text-sm", resolveClassName(className, state))
      }
