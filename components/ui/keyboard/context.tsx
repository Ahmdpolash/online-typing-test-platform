...
export function KeyboardProvider({
  ...
  useEffect(() => {
    ...
    const handleKeyUp = (event: KeyboardEvent) => {
      if (PHYSICAL_MODIFIER_CODES.has(event.code)) {
        modifiersDownRef.current.delete(event.code);
        // Release any non-modifier keys orphaned by meta chords
        if (modifiersDownRef.current.size === 0) {
          const orphans = Array.from(pressedKeysRef.current).filter(
            (k) => !PHYSICAL_MODIFIER_CODES.has(k) && k !== event.code
          );
          for (const k of orphans) {
            releaseKey(k, "physical");
          }
        }
      }
