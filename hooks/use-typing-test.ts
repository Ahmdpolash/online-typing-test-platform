...
export function useTypingTest({
  ...
  const wordPoolRef = useRef<{ hard: boolean; words: string[] } | null>(null);
  ...
  const resetTestWith = useCallback(
    ...
    [
      mode, quoteLength, wordOption, timeOption, punctuation, numbers, difficulty,
      buildWords, onFinished, onTypingActiveChange,
    ]
  );
  ...
  useEffect(() => {
    ...
    if (m === "quote") {
      ...
      buildWords(wc, { punctuation: p, numbers: n, difficulty: d }).then((w) =>
        setWords(w)
      );
    }
    if (m === "time") setTimeLeft(to);
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (e.key === "Enter" && e.shiftKey) {
        e.preventDefault();
        if (mode === "zen" && started && !finished) finishTest();
        return;
      }
      ...
      if (!started) {
        ...
        if (mode === "time") {
          ...
          timerRef.current = setInterval(() => {
            ...
            const snapWpm = elapsedMin > 0
              ? Math.round(correctCharsRef.current / 5 / elapsedMin)
              : 0;
            const snapRaw = elapsedMin > 0
    ...
    [
      finished, started, words, wordIndex, typed, wordInputs, mode, timeOption,
      resetTest, finishTest, onKeyHighlight, recordWordSnapshot, markTypingActive,
      onTypingActiveChange, onWrongKey, clearWordOrNavigateBack,
    ]
  );
  ...
  return {
    ...
    timeLeft, wordInputs, showControls, isFocused, resetting,
    isActivelyTyping, screenFade, wpm, accuracy,
    // Computed
    controlsVisible, showResults,
