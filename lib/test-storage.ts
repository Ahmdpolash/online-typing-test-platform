...
export const TEST_MODE_STORAGE_KEY = "tb-test-mode";
export const TIME_OPTION_STORAGE_KEY = "tb-time-option";
export const WORD_OPTION_STORAGE_KEY = "tb-word-option";
export const QUOTE_LENGTH_STORAGE_KEY = "tb-quote-length";
export const PUNCTUATION_STORAGE_KEY = "tb-punctuation";
export const NUMBERS_STORAGE_KEY = "tb-numbers";
export const DIFFICULTY_STORAGE_KEY = "tb-difficulty";
...
export function readStoredTimeOption(): TimeOption | undefined {
  ...
  if (raw === null) return;
  const n = Number(raw);
  if (!(Number.isFinite(n) && VALID_TIME_OPTIONS.includes(n as TimeOption))) return;
...
export function readStoredWordOption(): WordOption | undefined {
  ...
  if (raw === null) return;
  const n = Number(raw);
  if (!(Number.isFinite(n) && (VALID_WORD_OPTIONS as readonly number[]).includes(n))) return;
...
export function readStoredQuoteLength(): QuoteLength | undefined {
  ...
  if (!(raw && (VALID_QUOTE_LENGTHS as readonly string[]).includes(raw))) return;
...
export function readStoredDifficulty(): Difficulty | undefined {
  ...
  if (!(raw && (VALID_DIFFICULTIES as readonly string[]).includes(raw))) return;
